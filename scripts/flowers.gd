...
const DETAIL_DENSITY   := 0.95   # 0–1, higher = sparser
const DETAIL_SCALE     := 0.4    # base sprite scale
const DETAIL_SCALE_MIN := 0.75   # random size range min (multiplier)
const DETAIL_SCALE_MAX := 1.25   # random size range max (multiplier)
...
const DETAILS : Dictionary = {
	"grass": [
		preload("res://assets/sprites/deco/daisy.svg"),
		preload("res://assets/sprites/deco/tulip.svg"),
	],
	"sand":  [],
	"ocean": [],
}
