...
@export var fall_distance := 80.0
@export var fall_rotation := 90.0
...
func _on_pressed():
	...
	tween.set_parallel(true)
	tween.tween_property(self, "position:y", base_position.y + fall_distance, fall_time)
	tween.tween_property(self, "rotation", deg_to_rad(fall_rotation), fall_time)
	tween.tween_property(self, "scale", base_scale * 0.9, fall_time)
